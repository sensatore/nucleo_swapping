...
bool prepare_prd(natb* vett, natb quanti)
{
	...
	while (n && i < MAX_PRD) {
		...
		// vett e'gia un indirizzo fisico, non e'necessario trasformarlo
		paddr p = int_cast<paddr>(vett);
		// TESI
...
/// @brief Processo swapper
void swapper(natq)
{
	/**
	 * Non viene implementata la mutua esclusione mediante hd->mutex poiche'
	 * si suppone che l'hard disk sia dedicato esclusivamente alle operazioni 
	 * di swapping e non disponibile invece all'utente o per altre operazioni
	 * di sistema
	 */
	des_ata *hd = &hard_disk;
	for (;;) {
		des_swapper *ds = get_swap_op();
		natl lba_swap = ds->swap_lba;
		for (size_t i = 0; i < SWAP_FRAME_NUM; i++) {
			natb *vett = ptr_cast<natb>(ds->frame_addr[i]);

			if (ds->swap_type == SWAP_OUT_BLOCK || ds->swap_type == SWAP_OUT_NON_BLOCK) {
				if (ds->swap_frame[i]) { // 0 inserito nel caso di ottimizzazioni col bit D della pagina
					dmastarthd_out(hd, vett, lba_swap, BLOCKS_PER_FRAME);
					sem_wait(hd->sincr);
				}
			} else {
				dmastarthd_in(hd, vett, lba_swap, BLOCKS_PER_FRAME);
				sem_wait(hd->sincr);
			}

			lba_swap += BLOCKS_PER_FRAME;
		}

		terminate_swap_op(ds);
	}
}
...
bool hd_init()
{
	...
	// TESI
	id = activate_daemon(swapper, 0, MIN_EXT_PRIO + INTR_TIPO_HD, LIV_SISTEMA);
	flog(LOG_INFO, "Creato processo swapper: %u", id);
	if (id == 0xFFFFFFFF) {
		flog(LOG_ERR, "hd: impossibile creare proc. swapper");
		return false;
	}
	// TESI
